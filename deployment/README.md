...
> NOTE: The stack now uses **Next.js** for the frontend and **MongoDB** for persistence. Compose files and Nginx are configured to proxy `/api` to FastAPI and all other traffic to the Next.js server. Legacy PostgreSQL notes below are kept for reference only.
